...
def knn(num_points, k, topk_indices, features):
    ...
    with tf.name_scope('knn'):
        ...
        indices = concat([batch_indices, expand_dims(topk_indices, axis=3)], axis=3)  # (N, P, K, 2)
...
def edge_conv(points, features, num_points, K, channels, with_bn=True, activation='relu', pooling='average', name='edgeconv'):
    ...
    with tf.name_scope('edgeconv'):
        ...
        knn_fts_center = tile(expand_dims(fts, axis=2), multiples=[1, 1, K, 1])  # (N, P, K, C)
        knn_fts = concat([knn_fts_center, subtract(knn_fts, knn_fts_center)], axis=-1)  # (N, P, K, 2*C)
        ...
        for idx, channel in enumerate(channels):
            ...
            if with_bn:
                x = keras.layers.BatchNormalization(name='%s_bn%d' % (name, idx), momentum=0.7)(x)
            if activation:
        ...
        sc = keras.layers.Conv2D(channels[-1], kernel_size=(1, 1), strides=1, data_format='channels_last',
                                 use_bias=False if with_bn else True, kernel_initializer='glorot_normal', name='%s_sc_conv' % name)(expand_dims(features, axis=2))
        if with_bn:
            sc = keras.layers.BatchNormalization(name='%s_sc_bn' % name, momentum=0.7)(sc)
